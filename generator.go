...
// filterx 接口模版
type templateFilterxInterface struct {
	PackageName string
	GormHelper  bool
}
...
{{ if .GormHelper }}
	var (
		{{- range .Fields}}
			Field{{$.StructName}}{{.Name}} *Field = &Field{field:"{{.FilterxTag}}", sort:""}
		{{- end}}
	)
{{ end }}
...
func(f {{.StructName}}Filterx) isFilterx() {}
`

const filterxInterfaceTemplate = `// Code generated by filterx generator; DO NOT EDIT.
package {{.PackageName}}
...
{{ if .GormHelper }}
	...
	func (f *Field) SortDesc() *Field {
		f.sort = "DESC"
		return f
	}
	
	func (f *Field) SortAsc() *Field {
		f.sort = "ASC"
		return f
	}
	
	func (f *Field) apply(db *gorm.DB) *gorm.DB {
		if f.sort != "" {
			db = db.Order(fmt.Sprintf("%s %s", f.field, f.sort))
		}
		return db
	}
{{ end }}

type Filterx interface {
	isFilterx()
	{{ if .GormHelper }}
	GormHelper(db *gorm.DB, fields ...Field) *gorm.DB
	{{ end }}
}
`
...
func (cuf {{.StructName}}Filterx) GormHelper(db *gorm.DB, fields ...Field) *gorm.DB {
	{{- range .Fields}}
		if cuf.{{.Name}} != nil {
...
func (gen *Generator) Generate(in ...interface{}) error {
	...
	// 生成 filterx 接口
	f := templateFilterxInterface{
		PackageName: packageName,
		GormHelper:  gen.o.GormHelper,
	}
	err = gen.genFile("Filterx", f, filterxInterfaceTemplate, "interface")
	return nil
}
...
func (gen *Generator) genFile(structName string, data any, temp string, suffix string) error {
